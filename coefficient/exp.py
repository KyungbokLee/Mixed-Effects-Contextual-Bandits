...
parser.add_argument('--D', default=5.0, type=float)
...
if __name__ == '__main__':
    ...
    os.makedirs('./results/d=%d_K=%d_m=%d_D=%.1f_T=%d_c=%d_M=%d' % (d, K, m, D, T, c, M) ,exist_ok=True)
    alpha_set = [0.0,0.01,0.1,1.0,10.0]
    
    eval_C2UCB(d,K,m,D,alpha_set,T,M,c,1)
    eval_MECUCB1(d,K,m,D,alpha_set,T,M,c,1)
    eval_MECUCB2(d,K,m,D,alpha_set,T,M,c,1)
    ...
    for i, model in enumerate(['C2UCB', 'MECUCB1', 'MECUCB2']):
        with open(f'./results/d={d}_K={K}_m={m}_D={D:.1f}_T={T}_c={c}_M={M}/{model}.txt') as infile:
    ...
    plt.savefig(f'./results/d={d}_K={K}_m={m}_D={D:.1f}_T={T}_c={c}_M={M}/regrets_coeff_d={d}_K={K}_m={m}_D={D:.1f}_T={T}_c={c}_M={M}.pdf', bbox_inches='tight', pad_inches=0, dpi=300)
    plt.close()
    ...
    for i, model in enumerate(['C2UCB', 'MECUCB1', 'MECUCB2']):
        with open(f'./results/d={d}_K={K}_m={m}_D={D:.1f}_T={T}_c={c}_M={M}/{model}.txt') as infile:
            ...
            print(f'Best alpha for {model} : {alpha_set[best[model]]}.')

        mean = np.mean(results[model][best[model]]['errs'], axis=0)
        std = np.std(results[model][best[model]]['errs'], axis=0)
       
        plt.plot(np.arange(1,T+1),mean,label=labels[i], color=colors[i])
        plt.fill_between(np.arange(1,T+1), mean - 1.96/np.sqrt(M) * std, mean + 1.96/np.sqrt(M) * std, alpha=0.2, color=colors[i])
    ...
    plt.ylim((0,0.8))
    plt.savefig(f'./results/d={d}_K={K}_m={m}_D={D:.1f}_T={T}_c={c}_M={M}/errs_coeff_d={d}_K={K}_m={m}_D={D:.1f}_T={T}_c={c}_M={M}.pdf', bbox_inches='tight', pad_inches=0, dpi=300)
    plt.close()